...
async def settings_command(event):
    if not allowed(event.sender_id): return await event.reply("❌ You are not authorized.")
...
async def settings_menu(event):
    if not allowed(event.sender_id): return await event.answer("Not authorized", alert=True)
    ...
    if action == "video":
        ...
        buttons = [
            ...
            [Button.inline(f"Pixel: {s['pixel_format']}", b"edit:pixel_format")], [Button.inline("⬅️ Back", b"back:main")],
        ]
    elif action == "audio":
        text = "🔊 **Audio Settings**\n\nChoose an option to change:"
        buttons = [
            [Button.inline(f"Codec: {s['audio_codec']}", b"edit:audio_codec"), Button.inline(f"Bitrate: {s['audio_bitrate']}", b"edit:audio_bitrate")],
            [Button.inline(f"Channels: {s['audio_channels'] or 'Original'}", b"edit:audio_channels")], [Button.inline("⬅️ Back", b"back:main")],
        ]
    elif action == "output":
...
EDIT_LABELS = {
    "video_codec": "video codec (example: libx264, libx265, libsvtav1)", "crf": "CRF (0-51)",
    "preset": "preset (ultrafast to veryslow)", "video_bitrate": "video bitrate (example: 2500k). Send auto for CRF.",
    "pixel_format": "pixel format (example: yuv420p)", "audio_codec": "audio codec (example: aac, libopus, ac3)",
    "audio_bitrate": "audio bitrate (example: 128k, 192k, 320k)", "audio_channels": "audio channels (1, 2, 6) or original",
    "output_format": "output format (mkv or mp4)", "video_filter": "video filter (example: scale=-2:720). Send none to disable.",
...
async def cancel(event):
    ...
    if uid in pending_edits:
        pending_edits.pop(uid, None); return await event.reply("❌ Setting change cancelled.")
...
async def receive_setting(event):
    uid = event.sender_id
    if not allowed(uid): pending_edits.pop(uid, None); return
    key = pending_edits.pop(uid); value = (event.raw_text or "").strip()
    ...
    if not valid: return await event.reply("❌ Invalid value. Please try again.")
...
async def progress_message(message, title, done, total, start_time):
    now = time.monotonic(); elapsed = max(now - start_time, 0.001)
    percent = min(100, done / total * 100) if total else 0; speed = done / elapsed
    remaining = (total - done) / speed if speed > 0 and total else None
    bar_len = 12; filled = int(bar_len * percent / 100); bar = "█" * filled + "░" * (bar_len - filled)
...
async def media(event):
    ...
    if user_client is None:
        return await event.reply("❌ DOWNLOAD_SESSION is not configured. Add a Telegram user SESSION_STRING to enable reliable cross-DC file downloads.")
    ...
    work = Path(DOWNLOAD_DIR) / str(uid) / str(event.id); work.mkdir(parents=True, exist_ok=True)
    inp = work / safe; settings = await get_settings(uid); output_ext = settings.get("output_format", "mkv")
    out = work / f"{Path(safe).stem}.encoded.{output_ext}"; status = await event.reply("📥 DOWNLOADING\n[░░░░░░░░░░░░] 0.0%")
    job_id = await create_job(uid, safe, size)
    try:
        async with semaphore:
            await update_job(job_id, status="downloading"); download_started = time.monotonic(); last_download_update = 0.0
            ...
            async def encode_progress(percent, current_seconds, duration, remaining):
                elapsed = max(time.monotonic() - encode_started, 0.001); speed = current_seconds / elapsed if current_seconds else 0
                filled = int(12 * percent / 100); bar = "█" * filled + "░" * (12 - filled)
            ...
            await update_job(job_id, status="uploading", output_size=out.stat().st_size); upload_started = time.monotonic()
            ...
            await update_job(job_id, status="completed"); await status.edit(f"✅ COMPLETE\n📦 {fmt_bytes(out.stat().st_size)}"); await asyncio.sleep(3); await status.delete()
    except Exception as exc:
        log.exception("Job failed"); await update_job(job_id, status="failed", error=str(exc)[:500])
...
async def start_health_server():
    app = web.Application(); app.router.add_get("/health", health); runner = web.AppRunner(app); await runner.setup()
    port = int(os.environ.get("PORT", "8080")); site = web.TCPSite(runner, "0.0.0.0", port); await site.start(); log.info("Health server running on port %s", port); return runner

async def main():
    await init_db(); health_runner = await start_health_server()
