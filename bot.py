...
async def download_with_retry(client, source, output_path, total, progress_callback, max_retries=5):
    """Download Telegram media with resume/retry support instead of restarting at 0%."""
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            if total and actual < total:
                raise RuntimeError(f"Telegram download ended early ({actual} / {total} bytes)")
            await progress_callback(total or actual, total or actual)
            return
        except Exception as exc:
            last_error = exc
            log.warning(
                "Download interrupted (attempt %s/%s) at %s: %s",
                attempt, max_retries, fmt_bytes(downloaded), exc,
            )
            if attempt < max_retries:
                await asyncio.sleep(min(2 ** (attempt - 1), 10))

    raise RuntimeError(f"Telegram download failed after {max_retries} attempts: {last_error}")
...
async def media(event):
    ...
    try:
        async with semaphore:
            ...
            if not source:
                raise RuntimeError("The user session could not access the incoming Telegram message.")
            await download_with_retry(user_client, source, inp, size, download_progress)
